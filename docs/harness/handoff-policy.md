...
Claude Code may invoke `codex exec` automatically as the implementation step, **under the default sandbox only** (`read-only` / `workspace-write`), passing the handoff document as the brief (per `docs/decisions/ADR-003-allow-claude-code-to-invoke-codex-exec.md`). Automation does not remove the handoff: the written scope / out-of-scope / verification contract above is still produced and passed to Codex as the prompt. Codex CLI must be pinned to a crash-free version (see `.codex/CODEX_SETUP_NOTES.md`).
