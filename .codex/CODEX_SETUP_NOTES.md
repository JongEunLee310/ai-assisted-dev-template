...
## Prerequisites for automated `codex exec` (ADR-003)

`docs/decisions/ADR-003-allow-claude-code-to-invoke-codex-exec.md` lets Claude Code invoke `codex exec` automatically. Before relying on that, confirm all of the following — otherwise fall back to manual execution (ADR-002):
...
- **Default sandbox only.** Invoke under `read-only` / `workspace-write`. `--dangerously-bypass-approvals-and-sandbox` and `-s danger-full-access` are forbidden in automated workflows (ADR-003 point 2). If the default sandbox cannot run a task, stop and ask the human — never escalate.
